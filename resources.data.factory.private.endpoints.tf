...
resource "azurerm_private_endpoint" "pep" {
  ...
  private_service_connection {
    ...
    private_connection_resource_id = azurerm_data_factory.main_data_factory.id
    subresource_names              = ["datafactory"]
  }
}

#----------------------------------------------------------------------------
# DNS zone & records for Data Factory Private endpoints - Default is "false" 
#----------------------------------------------------------------------------
data "azurerm_private_endpoint_connection" "pip" {
  count               = var.enable_private_endpoint ? 1 : 0
  name                = azurerm_private_endpoint.pep.0.name
  resource_group_name = local.resource_group_name
  depends_on          = [azurerm_data_factory.main_data_factory]
}

resource "azurerm_private_dns_zone" "dns_zone" {
  count               = var.existing_private_dns_zone == null && var.enable_private_endpoint ? 1 : 0
  name                = var.environment == "public" ? "${azurerm_data_factory.main_data_factory.name}.${local.location}.datafactory.azure.net" : "${azurerm_data_factory.main_data_factory.name}.${local.location}.datafactory.azure.us"
  resource_group_name = local.resource_group_name
...
resource "azurerm_private_dns_a_record" "a_rec" {
  count               = var.enable_private_endpoint ? 1 : 0
  name                = azurerm_data_factory.main_data_factory.name
